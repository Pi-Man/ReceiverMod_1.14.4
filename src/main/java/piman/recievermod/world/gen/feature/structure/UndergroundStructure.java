...
public class UndergroundStructure extends Structure<NoFeatureConfig> {
    ...
    @Nullable
    @Override
    public BlockPos findNearest(@Nonnull World worldIn, ChunkGenerator<? extends GenerationSettings> chunkGenerator, BlockPos pos, int radius, boolean p_211405_5_) {
        int blockX = (pos.getX() >> 4)/BLOCK_SIZE;
        int blockZ = (pos.getZ() >> 4)/BLOCK_SIZE;

        Random blockRand = new Random(chunkGenerator.getSeed() + blockX << 16 + blockZ);

        int offsetX = blockRand.nextInt(BLOCK_SIZE - MIN_DISTANCE);
        int offsetZ = blockRand.nextInt(BLOCK_SIZE - MIN_DISTANCE);

        return new BlockPos((blockX * BLOCK_SIZE + offsetX) * 16, 0, (blockZ * BLOCK_SIZE + offsetZ) * 16);
    }
    ...
    public static class Start extends StructureStart {
        ...
        public void init(@Nonnull ChunkGenerator<?> generator, @Nonnull TemplateManager templateManagerIn, int chunkX, int chunkZ, @Nonnull Biome biomeIn) {
            ...
            try {
                IWorld world = (IWorld)ObfuscationReflectionHelper.findField(ChunkGenerator.class, "field_222540_a").get(generator);
        ...
        public void generateComponents(IWorld worldIn) {
            ...
            List<Set<Vector2D>> layerPoints = new ArrayList<>();
...
//                        }
                ...
                pieces.add(specialRooms.get(rand.nextInt(specialRooms.size())));
                pieces.add(specialRooms.get(rand.nextInt(specialRooms.size())));
                pieces.add(specialRooms.get(rand.nextInt(specialRooms.size())));
                ...
                while (!pieces.isEmpty()) {
                    ...
                    UndergroundPieces.AbstractPiece room = UndergroundPieces.AbstractPiece.fitPiece(templateManager, rand, sizeX, sizeZ, spawnCenter.add(0, -7 - 7 * h, 0), mask, 100, pieces.remove(0));
...
//                        }
                ...
                List<Vector2D> points = new ArrayList<>(layerPoints.get(h));
                ...
                try {
                    ...
                    if (points.size() == 2) {
                        ...
                        connectDoors(mask, hallwayBitMasks.get(h), points.get(1), points.get(0));
                    }
                }
                ...
                for (int i = 0; i < sizeX / 5; i++) {
                    for (int j = 0; j < sizeZ / 5; j++) {
                        if (hallwayBitMasks.get(h)[i][j]) {
                            ...
                            corridors.add(new UndergroundPieces.CorridorJunction(templateManager, rand, h, Rotation.NONE, i * 5 + spawnCenter.getX() - sizeX / 2, spawnCenter.getY() - 7 - 7 * h, j * 5 + spawnCenter.getZ() - sizeZ / 2 + 1, wallmask));
                        }
                    }
                }
...
//            }
        ...
        @Override
        public void generateStructure(@Nonnull IWorld worldIn, @Nonnull Random rand, @Nonnull MutableBoundingBox structurebb, @Nonnull ChunkPos pos) {
                ...
                if (w >= 0) {
                    ...
                    for (int i = 0; i < 5; i++) {
                        for (int j = -4; j < 1; j++) {
                            ...
                            //this.components.add(new UndergroundPieces.TestPiece(templateManager, door.getX() + i + room.pos.getX(), center.getY() - w*7 - 8, door.getZ() + j + room.pos.getZ()));
                        }
                    }
                }
            }
